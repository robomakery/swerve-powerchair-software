...
int dhr::odrive::getData(int id, T& value)
{
    ...
    if (result != LIBUSB_SUCCESS) {
	    if(result == LIBUSB_ERROR_TIMEOUT){
    		std:: cout << "* Error in transfering data to USB-Timeout! in getData" << std::endl;}
        return result;
    }
    	...
    	bool ack, int length, bool read, int address)
{
    ...
    int result = libusb_bulk_transfer(odrive_handle_, ODRIVE_OUT_EP,
    	    ...
    	    //std::cout <<  "* Transfering data start !" << std::endl;
   // if(result == LIBUSB_SUCCESS){std::cout <<  "* Successfull transfering data !" << std::endl;} 
    if (result != LIBUSB_SUCCESS) {
    		if(result == LIBUSB_ERROR_TIMEOUT){
    		std:: cout << "* Error in transfering data to USB-Timeout!" << std::endl;}
    		else if(result == LIBUSB_ERROR_INVALID_PARAM){
    		std:: cout << "* Error in transfering data to USB-the transfer size is larger than the operating system and/or hardware can support!" << std::endl;}    		
		else if(result == LIBUSB_ERROR_BUSY ){
    		std:: cout << "* Error in transfering data to USB-if called from event handling context!" << std::endl;}
    		else if(result == LIBUSB_ERROR_OVERFLOW ){
    		std:: cout << "* Error in transfering data to USB-if the device offered more data!" << std::endl;}
    		else if(result == LIBUSB_ERROR_NO_DEVICE ){
    		std:: cout << "* Error in transfering data to USB-LIBUSB_ERROR_NO_DEVICE!" << std::endl;}
    		else if(result == LIBUSB_ERROR_PIPE ){
    		std:: cout << "* Error in transfering data to USB-LIBUSB_ERROR_PIPE!" << std::endl;}
    		else if(result == LIBUSB_ERROR_IO){
    		std:: cout << "* Error in transfering data to USB-LIBUSB_ERROR_IO!" << std::endl;}
    		else if(result == LIBUSB_ERROR_ACCESS){
    		std:: cout << "* Error in transfering data to USB-LIBUSB_ERROR_NO_ACCESS!" << std::endl;}
    		else if(result == LIBUSB_ERROR_NOT_FOUND){
    		std:: cout << "* Error in transfering data to USB-LIBUSB_ERROR_NO_FOUND!" << std::endl;}
    		else if(result == LIBUSB_ERROR_INTERRUPTED){
    		std:: cout << "* Error in transfering data to USB-LIBUSB_ERROR_Interupt!" << std::endl;}	
		else{std:: cout << "* Error in transfering data to USB-Other error!" << std::endl;}
        ep_lock.unlock();  
        return result;
    }
    ...
    if (ack) {
    	...
    	//if(result == LIBUSB_SUCCESS){std::cout <<  "* Successfull reading data !" << std::endl;} 	
        if (result != LIBUSB_SUCCESS) {
    		if(result == LIBUSB_ERROR_TIMEOUT){
    		std:: cout << "* Error in reading data to USB-Timeout!" << std::endl;}
    		else if(result == LIBUSB_ERROR_INVALID_PARAM){
    		std:: cout << "* Error in reading data to USB-the transfer size is larger than the operating system and/or hardware can support!" << std::endl;}    		
		else if(result == LIBUSB_ERROR_BUSY ){
    		std:: cout << "* Error in reading data to USB-if called from event handling context!" << std::endl;}
    		else if(result == LIBUSB_ERROR_OVERFLOW ){
    		std:: cout << "* Error in reading data to USB-if the device offered more data!" << std::endl;}
    		else if(result == LIBUSB_ERROR_NO_DEVICE ){
    		std:: cout << "* Error in reading data to USB-LIBUSB_ERROR_NO_DEVICE!" << std::endl;}
    		else if(result == LIBUSB_ERROR_PIPE ){
    		std:: cout << "* Error in reading data to USB-LIBUSB_ERROR_PIPE!" << std::endl;}
    		else if(result == LIBUSB_ERROR_IO){
    		std:: cout << "* Error in reading data to USB-LIBUSB_ERROR_IO!" << std::endl;}
    		else if(result == LIBUSB_ERROR_ACCESS){
    		std:: cout << "* Error in reading data to USB-LIBUSB_ERROR_NO_ACCESS!" << std::endl;}
    		else if(result == LIBUSB_ERROR_NOT_FOUND){
    		std:: cout << "* Error in reading data to USB-LIBUSB_ERROR_NO_FOUND!" << std::endl;}
    		else if(result == LIBUSB_ERROR_INTERRUPTED){
    		std:: cout << "* Error in reading data to USB-LIBUSB_ERROR_Interupt!" << std::endl;}	
		else{std:: cout << "* Error in reading data to USB-Other error!" << std::endl;}
        ep_lock.unlock();
        return result;
    }
...
int dhr::getObjectByName(Json::Value odrive_json, std::string name, dhr::odrive_object *odo)
{
	...
	//if(ret==0){std::cout << name.c_str() << " found!\n";}
    if (ret) {
...
int dhr::getJson(dhr::odrive *endpoint, Json::Value *odrive_json)
{
    ...
    std::cout <<  "* Successfull parsing json!" << std::endl;
    return 0;
}
                ...
                std::string command, T &value)
{
	...
	//std:: cout << "* Trying to set data writeOdrive" << std::endl;	
