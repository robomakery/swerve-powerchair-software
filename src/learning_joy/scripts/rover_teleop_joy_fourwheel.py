...
def getAbsPos_vl(msg_vl):
	global abs_pos_vl
	if (msg_vl.data-2315) >= 0:	
		abs_pos_vl = (msg_vl.data-2315)/4096
	if (msg_vl.data-2315) < 0:
		abs_pos_vl = 1 - (2315-msg_vl.data)/4096
	pub_abs_rot_pos_vl.publish(abs_pos_vl)


def getAbsPos_vr(msg_vr):
	global abs_pos_vr
	if (msg_vr.data-2315) >= 0:	
		abs_pos_vr = (msg_vr.data-2315)/4096
	if (msg_vr.data-2315) < 0:
		abs_pos_vr = 1 - (2315-msg_vr.data)/4096
	pub_abs_rot_pos_vr.publish(abs_pos_vr)
	
def getAbsPos_hl(msg_hl):
	global abs_pos_hl
	if (msg_hl.data-2315) >= 0:	
		abs_pos_hl = (msg_hl.data-2315)/4096
	if (msg_hl.data-2315) < 0:
		abs_pos_hl = 1 - (2315-msg_hl.data)/4096
	pub_abs_rot_pos_hl.publish(abs_pos_hl)
	
def getAbsPos_hr(msg_hr):
	global abs_pos_hr
	if (msg_hr.data-2315) >= 0:	
		abs_pos_hr = (msg_hr.data-2315)/4096
	if (msg_hr.data-2315) < 0:
		abs_pos_hr = 1 - (2315-msg_hr.data)/4096
...
#Help variable
	...
	if abs(twist.linear.x)>0.06 or abs(twist.linear.y)>0.06 or abs(twist.angular.z)>0.06:
		pub_pos_vr.publish(14.33*wfr+mot_pos-90*abs_pos_vl);
